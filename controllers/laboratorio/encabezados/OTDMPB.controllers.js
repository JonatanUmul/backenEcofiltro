...
export const postOTDMPB = async(req, res)=>{
    const id_est= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO otdmpb(id_creador, id_est)Values(?, ?)';
...
export const putOTDMPB = async (req, res) => {
...
console.log('estado de front',estado,id)
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE otdmpb SET id_est = ?, fecha_cierre = ?, hora_cierre = ? WHERE id = ?';
