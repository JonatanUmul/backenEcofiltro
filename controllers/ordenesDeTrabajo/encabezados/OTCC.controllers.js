...
export const postOTCC = async(req, res)=>{
    const estado= 2;
    ...
    try{
        if(estado===''){
            ...
            const consulta='INSERT INTO otcc(id_est,id_creador)Values(?, ?)';
...
export const putOTCC = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE otcc SET id_est = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
