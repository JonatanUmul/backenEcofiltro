...
export const postOTPV = async(req, res)=>{
    const estado= 2;
    const id_creador=req.body.id_creador

    try{
        if(estado==='', id_creador===''){
            console.log('Uno o varios datos estan vacios')
        }
        else{
            const consulta='INSERT INTO otpv(id_est,id_creador)Values( ?, ?)';
...
export const putOTPV = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE otpv SET id_est = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
