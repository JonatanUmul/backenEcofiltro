...
export const postCKMM = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO ckmm (id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putCKMM = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE ckmm SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
