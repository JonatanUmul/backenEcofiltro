...
export const postCKTA = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO ckta(id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putCKTA = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE ckta SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
