...
export const postCKPHM = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO ckphm (id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putCKPHM = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE ckphm SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
