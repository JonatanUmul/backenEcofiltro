...
export const postCKBT = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO ckbt (id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putCKBT = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE ckbt SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
