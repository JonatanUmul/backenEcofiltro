...
export const postCKSCRUBBER = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO ckscrubber (id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putCKEXT = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE ckscrubber SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
